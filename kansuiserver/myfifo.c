...
void* myfifo_open_read(char* name)
{
	...
	ret=CreateMailslotA( tmp, 0, 1000*TIMEOUT, NULL);
...
void* myfifo_open_write(char* name)
{
	...
	ret=CreateFileA(tmp, GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
	if(ret==INVALID_HANDLE_VALUE)return NULL;
	return ret;
#endif
	return NULL;
